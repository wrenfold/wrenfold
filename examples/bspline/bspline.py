...
def weight(x: sym.Expr, i: int, k: int, knots: Sequence[sym.Expr]) -> sym.Expr:
    """
...
def B(
    ...
    i: int,
    k: int,
    degree_zero: Sequence[sym.Expr],
    knots: Sequence[sym.Expr],
) -> sym.Expr:
    """
...
def create_bases(
    ...
    knots: Sequence[sym.Expr],
    order: int,
    degree_zero_bases: Sequence[sym.Expr],
) -> list[sym.Expr]:
    """
...
def create_piecewise_polynomials(
    ...
    for i in range(0, order + (order - 1)):
        ...
        for j in range(0, order):
            ...
            key_and_value = list(zip(degree_zero_bases, values, strict=True))
        ...
        for interval_index, poly_piece in zip(range(0, order), interval_expressions, strict=True):
...
def create_polynomial_functions(
    x: sym.Expr, polynomials: Sequence[sym.Expr], order: int, is_cumulative: bool
...
def create_bspline_functions(order: int, visualize: bool = False) -> list[wf.FunctionDescription]:
    ...
    degree_zero_bases = sym.make_symbols([f"b_{i}_0" for i in range(0, number_of_knots - 1)])
