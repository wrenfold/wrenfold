...
def problem(
    ...
    for i in range(0, N):
        index_offset = i * 12
        index_next_offset = index_offset + 12
        state = trajectory[index_offset:index_next_offset, :]

        # Final state cost
        cost += desired_state_objective(
            state, desired_final_state, weights.final_state * i**2
        )

        # Dynamics cost
        if i < N - 1:
            index_next_next_offset = index_next_offset + 12
