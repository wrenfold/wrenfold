...
#ifdef WF_STACK_ALLOCATOR_INSTRUMENT
#include <fmt/core.h>
#endif
...
class stack_allocator {
 public:
  ...
  [[nodiscard]] constexpr void* allocate(const size_type n) noexcept {
    ...
    const auto ptr = p_;
#ifdef WF_STACK_ALLOCATOR_INSTRUMENT
    fmt::print("stack_allocator<{}, {}>::allocate({} [aligned: {}]) --> {}\n", Capacity, Alignment,
               n, n_aligned, static_cast<void*>(ptr));
#endif
    p_ += n_aligned;
...
#ifdef WF_STACK_ALLOCATOR_INSTRUMENT
      fmt::print("stack_allocator<{}, {}>::deallocate({}, {} [aligned: {}])\n", Capacity, Alignment,
                 static_cast<void*>(ptr), n, n_aligned);
#endif
