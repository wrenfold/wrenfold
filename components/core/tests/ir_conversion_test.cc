...
void check_output_expressions(const function_description& func,
  ...
  for (const auto& key_and_expected_expr : func.output_expressions()) {
    const auto& key = std::get<0>(key_and_expected_expr);
    const auto& expected_expr = std::get<1>(key_and_expected_expr);
